...
#define RADIAL_MENU_RADIUS (156)
#define RADIAL_MENU_START_ANGLE_DEG -90.0f // starting angle offset for the first icon (top of the circle)
...
static uint8_t main_menu_icons_cnt = sizeof(main_menu_icons) / sizeof(main_menu_icons[0]); // total number of main menu icons

ui_style_t ui_main_screen_style = UI_STYLE_RADIAL_MENU; // default style for the main screen
...
static lv_obj_t *ui_radial_menu_indicator[5] = {NULL}; // array to hold the radial menu dot indicators

static void ui_main_screen_card_menu_init(void);
static void ui_main_screen_radial_menu_init(void);
...
/**
 * @brief Initialize the main screen with the specified style (card menu or radial menu)
 *
 * @param style
 */
void ui_main_screen_init(ui_style_t style)
{
    ...
    if (style == UI_STYLE_CARD_MENU)
    {
        ui_main_screen_card_menu_init();
    }
    else if (style == UI_STYLE_RADIAL_MENU)
    {
        ui_main_screen_radial_menu_init();
    }
}
...
static void ui_main_screen_card_menu_init(void)
{
    ...
    for (uint8_t i = 0; i < main_menu_icons_cnt; i++)
    {
...
/**
 * @brief Initialize the main screen in radial menu style
 *
 */
static void ui_main_screen_radial_menu_init(void)
{
    ...
    lv_style_set_text_font(&style_main_screen, &lv_font_montserrat_20);
    lv_style_set_text_color(&style_main_screen, UI_COLOR_LILA);
    lv_style_set_bg_color(&style_main_screen, UI_COLOR_BLACK);
    lv_style_set_bg_opa(&style_main_screen, LV_OPA_COVER);

    lv_obj_add_style(ui_main_screen, &style_main_screen, 0);

    // -- create a container for the radial menu --
    ui_menu_container = lv_obj_create(ui_main_screen);
    lv_obj_set_size(ui_menu_container, SCREEN_WIDTH, SCREEN_HEIGHT);
    lv_obj_center(ui_menu_container);
    lv_obj_set_style_bg_opa(ui_menu_container, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(ui_menu_container, 0, 0);
    lv_obj_set_style_pad_all(ui_menu_container, 0, 0);

    float angle_step = 360.0f / main_menu_icons_cnt;

    for (uint8_t i = 0; i < main_menu_icons_cnt; i++)
    {
        float angle_deg = RADIAL_MENU_START_ANGLE_DEG + (angle_step * i);
        float angle_rad = angle_deg * (M_PI / 180.0f);

        lv_coord_t pos_x = SCREEN_CENTER_X + (RADIAL_MENU_RADIUS * cosf(angle_rad)) - 50;
        lv_coord_t pos_y = SCREEN_CENTER_Y + (RADIAL_MENU_RADIUS * sinf(angle_rad)) - 50;
        ...
        // create a small circular indicator below each button to indicate selection
        lv_obj_t *indicator = lv_obj_create(button);
        lv_obj_set_size(indicator, 8, 8);
        lv_obj_set_style_radius(indicator, LV_RADIUS_CIRCLE, 0);
        lv_obj_set_style_bg_color(indicator, UI_COLOR_LILA, 0);
        lv_obj_set_style_bg_opa(indicator, LV_OPA_TRANSP, 0); // initially making the indicators invisble
        lv_obj_set_style_border_width(indicator, 0, 0);
        lv_obj_align(indicator, LV_ALIGN_BOTTOM_MID, 0, 8);
        ui_radial_menu_indicator[i] = indicator;

        if (i == 0) // set the first button as the initially selected button
        {
            ui_main_menu_selected_btn = button;
            ui_main_menu_selected_btn_index = 0;
            lv_obj_set_style_bg_opa(ui_radial_menu_indicator[0], LV_OPA_COVER, 0); // make the first indicator visible

            lv_obj_set_style_img_opa(img, LV_OPA_COVER, 0); // make the first icon fully visible
        }
    }

    // -- setup the item label to display the selected menu item --
    ui_label_item = lv_label_create(ui_menu_container);
    lv_obj_center(ui_label_item);
    lv_obj_set_style_text_font(ui_label_item, &lv_font_montserrat_32, 0);
    lv_obj_set_style_text_color(ui_label_item, UI_COLOR_LILA, 0);
    lv_obj_set_style_text_align(ui_label_item, LV_TEXT_ALIGN_CENTER, 0);
    lv_label_set_long_mode(ui_label_item, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(ui_label_item, 200);
    lv_label_set_text(ui_label_item, main_menu_icons[ui_main_menu_selected_btn_index].text);
...
void ui_main_screen_knob_rotate(ui_knob_event_t event, ui_style_t style)
{
    ...
    {
        if (event == UI_KNOB_RIGHT)
        {
            if (ui_main_menu_selected_btn_index == 0)
                ui_main_menu_selected_btn_index = main_menu_icons_cnt - 1;
            else
                ui_main_menu_selected_btn_index--;
        }
        else if (event == UI_KNOB_LEFT)
        {
            ui_main_menu_selected_btn_index = (ui_main_menu_selected_btn_index + 1) % main_menu_icons_cnt;
        }

        // update the selected button based on the new index
        ui_main_menu_selected_btn = lv_obj_get_child(ui_menu_container, ui_main_menu_selected_btn_index);

        // hide all indicators first, also make all icons semi-transparent
        for (uint8_t i = 0; i < main_menu_icons_cnt; i++)
        {
            lv_obj_set_style_bg_opa(ui_radial_menu_indicator[i], LV_OPA_TRANSP, 0);
            lv_obj_set_style_img_opa(lv_obj_get_child(lv_obj_get_child(ui_menu_container, i), 0), LV_OPA_50, 0);
        }

        // show the indicator for the selected button and make its icon fully visible
        lv_obj_set_style_bg_opa(ui_radial_menu_indicator[ui_main_menu_selected_btn_index], LV_OPA_COVER, 0);
        lv_obj_set_style_img_opa(lv_obj_get_child(ui_main_menu_selected_btn, 0), LV_OPA_COVER, 0);

        // update the label to reflect the newly selected menu item
        if (ui_label_item)
            lv_label_set_text_static(ui_label_item, main_menu_icons[ui_main_menu_selected_btn_index].text);
    }
}
